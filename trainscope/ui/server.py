...
from trainscope.analysis import (
    get_spike_steps,
    read_arrow,
    read_arrow_sync,
...
def create_app(run_path: str, runs_root: str | None = None) -> FastAPI:
    ...
    async def _read_run_summary(run_dir: Path) -> dict[str, Any]:
        ...
        steps = await get_spike_steps(run_dir)
    ...
    async def get_compare(
        ...
        series: dict[str, list[dict[str, Any]]] = {}
        for n in names:
            rows = await read_arrow(by_name[n] / "global.arrow")
        ...
        for n in names:
            moe_rows = await read_arrow(by_name[n] / "moe.arrow")
    ...
    async def get_clusters() -> dict[str, Any]:
        ...
        async def _loss_band(member_names: list[str]) -> dict[str, Any]:
            ...
            for name in member_names:
                rows = await read_arrow(dir_by_name[name] / "global.arrow")
                baseline = sum(
    ...
    async def get_counterexample(
        ...
        query_sig = await run_signal_signature(query_dir)
        ...
        for name, run_dir in by_name.items():
            ...
            if sig["fired"]:
                continue
            spike_steps = await get_spike_steps(run_dir)
            if spike_steps:
                continue
        ...
        def _loss_series(run_dir: Path) -> list[dict[str, Any]]:
            rows = read_arrow_sync(run_dir / "global.arrow")
            return [
        ...
        for k in sorted(set(query_flat) | set(stable_flat)):
            if query_flat.get(k) == stable_flat.get(k):
                continue
            differing.append(
                {"field": k, "query_value": query_flat.get(k), "stable_value": stable_flat.get(k)}
            )
    ...
    async def get_global() -> list[dict]:
        return await read_arrow(rp / "global.arrow")
    ...
    async def get_moe() -> list[dict]:
        ...
        return await read_arrow(rp / "moe.arrow")
    ...
    async def get_spikes() -> list[dict[str, Any]]:
        steps = await get_spike_steps(rp)
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        try:
            ...
            while True:
                ...
                spikes = await get_spike_steps(rp)
