...
### Added
- **Replay view (C1).** `GET /api/replay` reads the run's `replay_config.json` (generated by `trainscope replay`) and maps each skipped batch index onto the training steps that consumed that batch via `batch_index` in `global.arrow`, so the UI shows which steps `SkippingDataLoader` will skip — not just which batch indices. The new Replay view (nav shortcut 7) shows the plan, the skip list, and red × markers on the loss curve at the affected steps.
- **`trainscope report` command (C2).** Generates a post-mortem case file for a run — spike story, fired early-warning signals with their lead, config, loss stats — as markdown or JSON, and a multi-run cluster report (same signal-signature grouping as the Runs view) for a root of runs. `--format markdown|json`, `--output <file>`.
- **Nearest-stable-run counterexample (A2).** `GET /api/counterexample?run=<name>` finds the stable run (no fired signal, no spikes) whose flattened config is closest to an exploding run, returning both loss series, the first signal's crossing step, and the differing config fields. The Runs view's "Why this & vs. stable" button renders the paired overlay and candidate-cause fields.
- **Cluster discriminant config traits (A1).** `/api/cluster` now reports, per cluster, the config fields every member shares that no stable run has (numeric/bool only), answering "why did THIS failure mode blow up and not the stable runs" at a glance; the Runs view renders each as a `distinct: <field>=<value>` badge.
- **Cluster normalized-loss band (A3).** Each cluster carries the per-step median + IQR band of member runs' losses normalized by their own first-40 baseline, drawn as a compact "common fate curve" on the cluster card.
- **First-signal crossing steps per cluster (A4).** `/api/cluster` reports each member run's first-signal crossing step; the Runs view shows `first signal at step N` badges, so "when did this failure mode start" is answerable per run.
- **Gradient-norm change in Diff view (C4).** `/api/diff` reports each layer's `grad_l2_norm` change between the two selected steps (null when either step lacks a norm); the Top diverged layers list gains a color-coded `Δgrad` badge.
- **Live MoE streaming (C3).** The `/ws` WebSocket now streams `moe.arrow` alongside `global.arrow` (`moe` full message on connect, `moe_delta` appends), and `ExpertUtilization` reads from `RunContext` instead of fetching once, so routing concentration follows a live run.
- **Remote run trees via fsspec (C5).** `trainscope ui` and `trainscope report` accept `s3://`, `gs://`, and other fsspec URIs for `--run`/`--runs`; remote trees are materialized into a local cache before analysis. Report run names come from the config's `run_name`, not the directory.
- **Attention-collapse verification script (B1).** `scripts/verify_attention_collapse_signal.py` tests the third candidate architecture signal (per-head max attention weight and normalized attention entropy) with the same stable-control vs LR-ramp methodology as the MoE/addressor experiments. Thresholds are candidates pending calibration against the control's observed ceiling.
- **Signal-signature analysis module.** The early-warning signature logic (durable MAD+3σ crossing, chronological first-signal ordering, objective explosion definition) moved from a `create_app` closure into `trainscope.analysis`, shared by `/api/cluster`, `/api/counterexample`, and `trainscope report`.
- **Frozen plugin contract (D3).** The stability scope now treats `AnomalyDetector` (`update`, `warmup`), `MetricPlugin` (`name`, `compute`), and the plugin-metrics table columns (`step`/`plugin`/`metric`/`value`) as frozen, pinned by `tests/test_plugin_contract.py`. README's "not yet a frozen contract" caveat removed.

### Changed
- `ui` and `report` CLI commands accept local paths or fsspec URIs for `--run`/`--runs` (previously local-only).

### Tests
- Legacy-run compatibility (D1), Arrow schema-evolution compatibility (D2), and config round-trip equivalence (D4) suites added, pinning the stability scope in code.
